...
<header class="sticky top-0 z-50 border-b bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60">
    ...
    <?php
        ...
        $avatarUrl = $avatarUrl ? "uploads/profile_image/" . htmlspecialchars($avatarUrl) : "assets/img/default.png";
    ?>
    <div class="flex items-center gap-2">
        <a href="index.php?page=profile">
            <img src="<?= $avatarUrl ?>" onerror="this.onerror=null; this.src='assets/img/default.png';"
                 alt="avatar" class="w-8 h-8 rounded-full border" />
        </a>
        <a href="index.php?page=profile">
            <span class="text-sm font-medium text-slate-700"><?= htmlspecialchars($user['username'] ?? '') ?></span>
        </a>
